...
namespace
{
/// select chambers for full info by-layer debug printout
bool isDebugId(DetId &id)
{
  if (id.det() != DetId::Muon ) return false;

  static std::set<uint32_t> ids;

  if (ids.empty()) // fill just once on the 1st call
  {
    //ids.insert( DTChamberId(0, 2, 1).rawId() );
    //ids.insert( DTChamberId(0, 2, 12).rawId() );
    //ids.insert( DTChamberId(0, 1, 2).rawId() );
    //ids.insert( DTChamberId(1, 2, 1).rawId() );
    //ids.insert( DTChamberId(0, 4, 14).rawId() );

    ids.insert( DTChamberId(2, 4, 2).rawId() );
    ids.insert( DTChamberId(2, 4, 5).rawId() );
    ids.insert( DTChamberId(2, 1, 5).rawId() );
    ids.insert( DTChamberId(1, 4, 2).rawId() );
    ids.insert( DTChamberId(2, 4, 3).rawId() );
    ids.insert( DTChamberId(2, 4, 6).rawId() );
    ids.insert( DTChamberId(-1, 4, 3).rawId() );
    ids.insert( DTChamberId(-1, 4, 2).rawId() );
    ids.insert( DTChamberId(-2, 2, 2).rawId() );
    ids.insert( DTChamberId(0, 1, 2).rawId() );
    ids.insert( DTChamberId(0, 4, 5).rawId() );
    ids.insert( DTChamberId(2, 2, 5).rawId() );
    ids.insert( DTChamberId(0, 2, 4).rawId() );
    std::cout<<"debug IDs LUT filled with "<<ids.size()<<" ids"<<std::endl;
  }

  if (id.subdetId() == MuonSubdetId::DT)
  {
    const DTChamberId cid(id.rawId());
    if (ids.find(cid.rawId()) != ids.end()) return true;
  }
  else if (id.subdetId() == MuonSubdetId::CSC)
  {
    const CSCDetId cid(id.rawId());
    if (ids.find(cid.rawId()) != ids.end()) return true;
  }
  return false;
}
...
const Float_t LayerAlignEntry_default_float = 1111.0;

struct LayerAlignEntry
{
	Char_t m_nlayers; // Count of layers processed
	//Char_t m_endCap; // -1, 1 specifies endcap for CSCs; 0 specifies DT
	Bool_t m_is_dt;
	Bool_t m_is_plus;
	Char_t m_station;
	Char_t m_ring_wheel;
	Char_t m_sector;
	Char_t m_q; // Charge
	Float_t m_pt; // Transverse Momentum
	Float_t m_pz; // Z Momentum
	Float_t m_res_x; // Chamber x resid
	Float_t m_res_y; // Chamber y resid
	Float_t m_res_dxdz; // Chamber dxdz resid
	Float_t m_res_dydz; // Chamber dydz resid
	Float_t m_eres_x;
	Float_t m_eres_y;
	Float_t m_eres_dxdz;
	Float_t m_eres_dydz;
	...
	Float_t m_track_x;
	Float_t m_track_y;
	Float_t m_track_dxdz;
	Float_t m_track_dydz;
	Float_t m_traj_z [nLayers]; //Trajectory on Surface z
	Float_t m_traj_x [nLayers]; //Trajectory on Surface x
	Float_t m_traj_y [nLayers]; //Trajectory on Surface y
	Float_t m_hit_x [nLayers]; //Hit x (represents x or y depending on layer)
	Float_t m_ehit_x [nLayers]; //Hit x Error

	LayerAlignEntry():
	m_nlayers(0),
	//m_endCap(0), // -1, 1 specifies endcap for CSCs(0), 0 specifies DT
	m_station(0),
	m_ring_wheel(0),
	m_sector(0),
	m_q(0), // Charge
	m_pt(LayerAlignEntry_default_float), // Transverse Momentum
	m_res_x(LayerAlignEntry_default_float), // Chamber x resid
	m_res_y(LayerAlignEntry_default_float), // Chamber y resid
	m_res_dxdz(LayerAlignEntry_default_float), // Chamber dxdz resid
	m_res_dydz(LayerAlignEntry_default_float), // Chamber dydz resid
	m_eres_x(LayerAlignEntry_default_float),
	m_eres_y(LayerAlignEntry_default_float),
	m_eres_dxdz(LayerAlignEntry_default_float),
	m_eres_dydz(LayerAlignEntry_default_float),
	m_seg_x(LayerAlignEntry_default_float),
	m_seg_y(LayerAlignEntry_default_float),
	m_seg_dxdz(LayerAlignEntry_default_float),
	m_seg_dydz(LayerAlignEntry_default_float),
	m_track_x(LayerAlignEntry_default_float),
	m_track_y(LayerAlignEntry_default_float),
	m_track_dxdz(LayerAlignEntry_default_float),
	m_track_dydz(LayerAlignEntry_default_float)
	{
		for(int i=0; i<nLayers; ++i)
		{
			m_traj_z[i] = LayerAlignEntry_default_float;
			m_traj_x[i] = LayerAlignEntry_default_float;
			m_traj_y[i] = LayerAlignEntry_default_float;
			m_hit_x[i] = LayerAlignEntry_default_float;
			m_ehit_x[i] = LayerAlignEntry_default_float;
		}
	}

	LayerAlignEntry & operator = (LayerAlignEntry x)
	{
		//m_endCap = x.m_endCap;
		m_is_dt = x.m_is_dt;
		m_is_plus = x.m_is_plus;
		m_station = x.m_station;
		m_ring_wheel = x.m_ring_wheel;
		m_sector = x.m_sector;
		m_q = x.m_q; // Charge
		m_pt = x.m_pt; // Transverse Momentum
		m_res_x = x.m_res_x; // Chamber
		m_res_y = x.m_res_y;
		m_res_dxdz = x.m_res_dxdz;
		m_res_dydz = x.m_res_dydz;
		m_eres_x = x.m_eres_x;
		m_eres_y = x.m_eres_y;
		m_eres_dxdz = x.m_eres_dxdz;
		m_eres_dydz = x.m_eres_dydz;
		m_seg_x = x.m_seg_x;
		m_seg_y = x.m_seg_y;
		m_seg_dxdz = x.m_seg_dxdz;
		m_seg_dydz = x.m_seg_dydz;
		m_track_x = x.m_track_x;
		m_track_y = x.m_track_y;
		m_track_dxdz = x.m_track_dxdz;
		m_track_dydz = x.m_track_dydz;
		for(int i=0; i<nLayers; ++i)
		{
			m_traj_z[i] = x.m_traj_z[i];
			m_traj_x[i] = x.m_traj_x[i];
			m_traj_y[i] = x.m_traj_y[i];
			m_hit_x[i] = x.m_hit_x[i];
			m_ehit_x[i] = x.m_ehit_x[i];
		}
		return *this;
	};

	bool isDefault(Float_t f)
	{
		return f == LayerAlignEntry_default_float;
	}
};
...
private:
  ...
  void layersDebugDump(const Trajectory *traj, const reco::Track* track, MuonResidualsFromTrack &mrft);

  bool isTrackFiducialInMuonChamber(DetId &id, double trk_x, double trk_y, double trk_dxdz, double trk_dydz);

  bool residualsCheck(MuonResidualsFromTrack &mrft);
  bool dtStation123Check(MuonResidualsFromTrack &mrft, DetId id);
  bool dtStation4Check(MuonResidualsFromTrack &mrft, DetId id);
  bool cscCheck(MuonResidualsFromTrack &mrft, DetId id);
  ...
  // layer residual
  LayerAlignEntry m_layer_align_entry;
...
void MuonAlignmentFromReference::bookNtupleLayers()
{
	edm::Service<TFileService> fs;

    m_ttree_CSC_layers = fs->make<TTree>("csc_layer_ttree", "csc_layer_ttree");
	m_ttree_CSC_layers->Branch("charge", &(layerData.charge), "charge/i");
	m_ttree_CSC_layers->Branch("nEvent", &(layerData.nEvent), "nEvent/i");
	m_ttree_CSC_layers->Branch("nlayers", &(layerData.nlayers), "nlayers/i");

	m_ttree_CSC_layers->Branch("nDT", &(layerData.nDT), "nDT/i");
	m_ttree_CSC_layers->Branch("nCSC", &(layerData.nCSC), "nCSC/i");
	m_ttree_CSC_layers->Branch("nTracker", &(layerData.nTracker), "nTracker/i");
	
    m_ttree_CSC_layers->Branch("select", &(layerData.select), "select/O");


	m_ttree_CSC_layers->Branch("endcap", &(layerData.endcap), "endcap/b");
	m_ttree_CSC_layers->Branch("station", &(layerData.station), "station/b");
	m_ttree_CSC_layers->Branch("ring", &(layerData.ring), "ring/b");
	m_ttree_CSC_layers->Branch("chamber", &(layerData.chamber), "chamber/b");
	...
	m_ttree_CSC_layers->Branch("hit_y", &(layerData.v_hity), "hit_y[6]/F");
...
void MuonAlignmentFromReference::run(const edm::EventSetup& iSetup, const EventInfo &eventInfo)
{
    ...
    for (ConstTrajTrackPairCollection::const_iterator trajtrack = trajtracks.begin();  trajtrack != trajtracks.end();  ++trajtrack)
    {
      ...
      if (m_minTrackPt < track->pt()  &&  track->pt() < m_maxTrackPt && m_minTrackP < track->p()  &&  track->p() < m_maxTrackP)
      {
        ...
        if ( fabs(track->dxy(eventInfo.beamSpot_.position())) < m_maxDxy )
        {
          ...
          layerData.nEvent = m_counter_events;

          MuonResidualsFromTrack muonResidualsFromTrack(iSetup, magneticField, globalGeometry, prop, traj, track, m_alignableNavigator, 1000., &layerData, m_ttree_CSC_layers);
          //MuonResidualsFromTrack muonResidualsFromTrack(iSetup, magneticField, globalGeometry, prop, traj, track, m_alignableNavigator, 1000., m_ttree_CSC_layers);
          std::cout << "JUST AFTER muonResidualsFromTrack" << std::endl;

          //layerData.eta = track->eta();
          //layerData.pt = track->pt();
          //layerData.pz = track->pz();
  ...
  std::cout << "****** EVENT STOP *******" << std::endl << std::endl;
}


bool MuonAlignmentFromReference::residualsCheck(MuonResidualsFromTrack &mrft)
{
	if (mrft.trackerNumHits() >= m_minTrackerHits)
	{
		...
		if (mrft.normalizedChi2() < m_maxTrackerRedChi2)
		{
			...
			if (m_allowTIDTEC  ||  !mrft.contains_TIDTEC())
			{
				...
				if ((int)mrft.chamberIds().size() >= m_minNCrossedChambers)
				{
...
bool MuonAlignmentFromReference::dtStation123Check(MuonResidualsFromTrack &mrft, DetId id)
{
	if (m_doDT && id.subdetId() == MuonSubdetId::DT  && DTChamberId(id.rawId()).station() != 4)
	{
		MuonChamberResidual *dt13 = mrft.chamberResidual(DTChamberId(id.rawId()), MuonChamberResidual::kDT13);
		MuonChamberResidual *dt2 = mrft.chamberResidual(DTChamberId(id.rawId()), MuonChamberResidual::kDT2);
...
bool MuonAlignmentFromReference::dtStation4Check(MuonResidualsFromTrack &mrft, DetId id)
{
	if (m_doDT && id.subdetId() == MuonSubdetId::DT && DTChamberId(id.rawId()).station() == 4)
	{
		MuonChamberResidual *dt13 = mrft.chamberResidual(DTChamberId(id.rawId()), MuonChamberResidual::kDT13);
...
bool MuonAlignmentFromReference::cscCheck(MuonResidualsFromTrack &mrft, DetId id)
{
	if (m_doCSC && id.subdetId() == MuonSubdetId::CSC)
	{
		MuonChamberResidual *csc = mrft.chamberResidual(id, MuonChamberResidual::kCSC);
		m_counter_csc++;
		if (csc != NULL)
		{
			m_counter_cscvalid++;
...
void MuonAlignmentFromReference::processMuonResidualsFromTrack(MuonResidualsFromTrack &mrft)
{
	if(residualsCheck(mrft))
	{
		...
		for (std::vector<DetId>::const_iterator chamberId = chamberIds.begin();  chamberId != chamberIds.end();  ++chamberId)
		{
			...
			// DT station 1,2,3
			if(dtStation123Check(mrft, *chamberId))
			{
			...
			// DT 4th station
			else if(dtStation4Check(mrft, *chamberId))
			{
				MuonChamberResidual *dt13 = mrft.chamberResidual(*chamberId, MuonChamberResidual::kDT13);
				std::map<Alignable*,MuonResidualsTwoBin*>::const_iterator fitter = m_fitters.find(dt13->chamberAlignable());
				if (fitter != m_fitters.end())
			...
			// CSC
			else if(cscCheck(mrft, *chamberId))
			{
				MuonChamberResidual *csc = mrft.chamberResidual(*chamberId, MuonChamberResidual::kCSC);
				Alignable *ali = csc->chamberAlignable();

				CSCDetId cid(ali->geomDetId().rawId());
				if (m_combineME11  &&  cid.station() == 1  &&  cid.ring() == 4)  ali = m_me11map[ali];
...
void MuonAlignmentFromReference::fitAndAlign()
{
  ...
  std::cout << "***** start fitAndAlign" << std::endl;
...
//        bool successful_fit = false;
        ...
        std::cout << "***** loop over alignables 7 " << fitter->second->type() << std::endl;
...
void MuonAlignmentFromReference::selectResidualsPeaks() // FIXME here the name is selectResidualsPeaks, in MuonResidualsFitter.cc it is selectPeakResiduals
{
  ...
  for (std::vector<unsigned int>::const_iterator index = m_indexes.begin();  index != m_indexes.end();  ++index)
  {
    ...
    std::cout << "++++++ JUST BEFORE selectPeakResiduals" << std::endl;
    fitter->selectPeakResiduals(m_peakNSigma, nvar, vars_index);
    
    std::cout << "                   selectPeakResiduals m_peakNSigma: " << m_peakNSigma << " nvar: " << nvar << std::endl;
