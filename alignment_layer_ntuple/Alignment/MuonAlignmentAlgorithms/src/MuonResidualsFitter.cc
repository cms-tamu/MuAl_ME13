...
void MuonResidualsFitter::initialize_table()
{
  ...
  else
  {
    ...
    //nja-std::cout << "Initializing convolution look-up table (takes a few minutes)..." << std::endl;

    for (int gsbin = 0;  gsbin < MuonResidualsFitter_numgsbins;  gsbin++)
    {
      double gammaoversigma = double(gsbin) * MuonResidualsFitter_gsbinsize;
      //nja-std::cout << "    gsbin " << gsbin << "/" << MuonResidualsFitter_numgsbins << std::endl;
      for (int tsbin = 0;  tsbin < MuonResidualsFitter_numtsbins;  tsbin++)
      {
    ...
    //nja-std::cout << "Initialization done!" << std::endl;
  }
}
...
//  MuonResidualsFitter_TMinuit->SetPrintLevel(m_printLevel);
  ...
  //nja-std::cout << "++++++++++ Dofit 4" << std::endl;
  MuonResidualsFitter_TMinuit->mnexcm("MIGRAD", arglist, 1, ierflg);
  //nja-std::cout << "++++++++++ Dofit 5" << std::endl;
...
//nja-std::cout << "MuonResidualsFitter" << std::endl;
...
void MuonResidualsFitter::computeHistogramRangeAndBinning(int which, int &nbins, double &a, double &b) 
{
  ...
  //nja-std::cout<<"   quantiles: ";  for (int i=0;i<n_quantiles;i++) //nja-std::cout<<quantiles[i]<<" "; //nja-std::cout<<std::endl;
  ...
  //nja-std::cout<<"   optimal h="<<hbin<<" nbins="<<nbins<<std::endl;
}
...
void MuonResidualsFitter::histogramChi2GaussianFit(int which, double &fit_mean, double &fit_sigma)
{
  ...
  //nja-std::cout<<" h("<<nbins<<","<<a<<","<<b<<") mu="<<fit_mean<<" sig="<<fit_sigma<<std::endl;
...
{
//  //nja-std::cout<<"MuonResidualsFitter::selectPeakResiduals nsigma = "<<nsigma<<" nvar = "<<nvar << std::endl;
//  //nja-std::cout << "                                       numResiduals(): " << numResiduals() << std::endl;
  
  for (int i=0; i<nvar; ++i) std::cout<<vars[i]<<" ";
  //nja-std::cout<<std::endl;
  ...
  //nja-std::cout<<" N residuals "<<nbefore<<" ~ "<<(size_t) std::count(m_residuals_ok.begin(), m_residuals_ok.end(), true)<<std::endl;
//  //nja-std::cout << "+++++ JUST after N residuals" << std::endl;
  //just to be sure (can't see why it might ever be more then 10)
  assert(nvar<=10 && nvar>0);

  std::vector<double*>::iterator r = m_residuals.begin();
//  //nja-std::cout << "+++++ JUST before if (nvar==1)" << std::endl;
  // it's awkward, but the 1D case has to be handled separately
  if (nvar==1)
  {
    //nja-std::cout << "1D case" << std::endl;
    ...
    //nja-std::cout<<" N residuals "<<nbefore<<" -> "<<numResiduals()<<std::endl;
    return;
  } // end 1D case
  
//  //nja-std::cout << "+++++ JUST after if (nvar==1)" << std::endl;
  
  //nja-std::cout << "D>1 case" << std::endl;
  // initialize and run the robust estimator for D>1
  TRobustEstimator re(nbefore+1, nvar);
  //nja-std::cout << "nbefore " << nbefore << " nvar " << nvar << std::endl;
  r = m_residuals.begin();
  //nja-std::cout << "+++++ JUST before loop while (r != m_residuals.end())" << std::endl;
  int counter1 = 0;
  while (r != m_residuals.end())
  {
//    //nja-std::cout << "+++++ Inside loop while (r != m_residuals.end())" << std::endl;
    double *row = new double[nvar];
    for (int v = 0; v<nvar; v++)  row[v] = (*r)[ vars[v] ];
    //nja-std::cout << "counter1 " << counter1 << "row[0] " << row[0] << " row[1] " << row[1] << std::endl;
    re.AddRow(row);
//    delete[] row;
    ++r;
    counter1++;
  }
  //nja-std::cout << "counter1 " << counter1 << std::endl;
  //nja-std::cout << "+++++ JUST after loop while (r != m_residuals.end())" << std::endl;
  re.Evaluate();
  //nja-std::cout << "+++++ JUST after re.Evaluate()" << std::endl;
  ...
  Cov.Invert();
  //nja-std::cout << "+++++ JUST after Cov.Invert();" << std::endl;
...
void MuonResidualsFitter::correctBField(int idx_momentum, int idx_q)
{
  ...
  std::cout << "bin_width: " << bin_width << " minpt: " << min_pt << std::endl;
  ...
  for (size_t i = 0; i < m_residuals.size(); i++)
  {
    ...
    std::cout << (int)floor(1./fabs(m_residuals[i][idx_momentum])/bin_width) << " " << m_residuals[i][idx_momentum] << std::endl;
...
void MuonResidualsFitter::eraseNotSelectedResiduals()
{
  ...
  //nja-std::cout << "residuals sizes: all=" << m_residuals.size()<<" good="<<n_ok<<std::endl;
  int iok=0;
  for (size_t i = 0; i < m_residuals.size(); i++)
  {
    if (!m_residuals_ok[i]) continue;
  ...
  //nja-std::cout << "residuals size after eraseNotSelectedResiduals =" << m_residuals.size()<<"  ok size="<<m_residuals_ok.size()<<std::endl;
}
